...
from config import (
    ...
    HIGH_CONF_THRESHOLD, PHASE2_TEST_ACCURACY,
)
...
def load_best_model() -> tuple[nn.Module, dict]:
    ...
    num_price_features = checkpoint["num_price_features"]
    num_sent_features  = checkpoint["num_sent_features"]
...
def collect_predictions(model: nn.Module, loader) -> dict:
    ...
    for X_price, X_sent, y_regime, y_transition, stock_ids in loader:
...
def evaluate_regime(results: dict) -> dict:
    ...
    if high_conf_count > 10:
        high_conf_acc = accuracy_score(labels[high_conf_mask],
                                        preds[high_conf_mask])
...
def print_report(regime_metrics: dict, trans_metrics: dict,
    ...
    print("\n" + "=" * 70)
    print("  PHASE 4 EVALUATION — SENTIMENT-ENRICHED MODEL vs PHASE 2 BASELINE")
    print("=" * 70)

    acc = regime_metrics["accuracy"]
    acc_pass = acc >= MIN_TEST_ACCURACY
    improvement = (acc - PHASE2_TEST_ACCURACY) * 100
    sign = "+" if improvement >= 0 else ""

    print(f"\n  -- REGIME CLASSIFICATION ------------------------------------")
    print(f"  Overall Test Accuracy:     {acc:.4f} ({acc*100:.1f}%)  "
          f"{'  ' + PASS if acc_pass else '  ' + FAIL} "
          f"(threshold: {MIN_TEST_ACCURACY*100:.0f}%)")
    print(f"  Phase 2 Baseline:          {PHASE2_TEST_ACCURACY:.3f} ({PHASE2_TEST_ACCURACY*100:.1f}%)")
    ...
    print(f"  Transition Recall:    {trans_recall:.4f}  "
          f"{'  ' + PASS if trans_pass else '  ' + FAIL} "
          f"(threshold: {MIN_TRANSITION_RECALL})")
    print(f"  Transition Precision: {trans_metrics['precision']:.4f}")
    print(f"  Transition F1:        {trans_metrics['f1']:.4f}")
    print(f"  Transition Rate:      {trans_metrics['pos_rate']:.3f}")

    print(f"\n  -- CALIBRATION ----------------------------------------------")
    print(f"  Expected Calibration Error (ECE): {calibration['ece']:.4f}")

    hard_checks = [
        (f"Test Accuracy >= {MIN_TEST_ACCURACY*100:.0f}%",    acc_pass),
        (f"High-Conf Accuracy >= {MIN_HIGH_CONF_ACCURACY*100:.0f}%", hc_pass),
        (f"Min Per-Class F1 >= {MIN_PER_CLASS_F1}",           f1_pass),
        (f"Transition Recall >= {MIN_TRANSITION_RECALL}",      trans_pass),
    ]
    ...
    print("\n" + "=" * 70)
    if all_pass:
        print(f"  VERDICT: GO ({passed_count}/4) — Sentiment-enriched model "
              f"beats Phase 2. Proceed to Phase 5.")
    else:
        print(f"  VERDICT: NO-GO ({passed_count}/4) — Model needs improvement "
              f"before proceeding.")
    print("=" * 70 + "\n")
...
def plot_evaluation_dashboard(regime_metrics: dict, trans_metrics: dict,
    ...
    fig = make_subplots(
        ...
        subplot_titles=[
            "Confusion Matrix",
            "Per-Class F1 (Phase 4 vs Phase 2 Threshold)",
            "Calibration Plot",
    ...
    fig.add_hline(y=MIN_PER_CLASS_F1, line_dash="dash", line_color="#FF6D00",
                  annotation_text=f"Threshold ({MIN_PER_CLASS_F1})",
                  row=1, col=2)
    ...
    fig.update_layout(
        ...
        height=800,
        title=dict(
            text="<b>Phase 4 — Sentiment-Enriched Model Evaluation</b>",
    ...
    path = PLOTS_DIR / "phase4_evaluation_dashboard.html"
...
def main():
    print("\n" + "=" * 70)
    print("  PHASE 4 — SENTIMENT-ENRICHED MODEL EVALUATION")
    print("=" * 70 + "\n")
    ...
    plot_evaluation_dashboard(regime_metrics, trans_metrics, calibration,
                              results)
    print(f"  Dashboard saved: {PLOTS_DIR}/phase4_evaluation_dashboard.html\n")
